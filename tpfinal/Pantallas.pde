...
Pantallas(){
  ...
  Pantalla[18] = loadImage("Cuevadentro.jpg");
...
// Cargar texto de las pantallas // 
  ...
  textos[17] = "¡Persevera y triunfaras! despues de caminar por \n horas y horas, reconoces el lugar  donde te encontraste por\n primera vez con el oso.¡Este es el camino a casa! ";
  textos[12] = "caminas sin parar, hasta que llegas a una cueva que no habias \n visto antes";
  textos[13] = "Pero antes de que te puedas dar veulta, el oso sale disparado \n de la cueva,ERA SU GUARIDA. intentas pero ya es\n demasiado tarde, el oso te tomo por sorpresa.  ";
  ...
  textos[9] = "cuando se hace de noche, espias por \n la ventana y ves que el oso no esta.";
  textos[16] = "un texto para completar";
  textos[10] = "luego de unos pasos, vez unos ojos blancos \n asomandose entre los arboles.\n ¡ES EL CHUPACABRAS!";
  textos[18] = "¡Llegaste al final, puedes volver a casa\n lo has logrado !";
  textos[20] = "Moriste";  
...
void mostrarCuadro(int index){
  image(Pantalla[17], 0, height / 6 + height /2 , width, height / 4  );
  pushStyle();
  textAlign(CENTER);
  textSize(width / 40);
  text(textos[index], width / 2, height / 4 + height / 2);
  popStyle();
}
}
